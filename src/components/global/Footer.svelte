...
<footer class="footer">
  <div class="row1">
    ...
    <div class="avaiableLanguagesContainer">
      <p>{firstLetterUppercase($DataLanguages["common"].languages)} :</p>
      {#each avaiblesLanguages as langues}
        <p
          class={`langues ${langues == $currentLanguages ? "active" : ""}`}
          on:click={() => changeLanguages(langues)}
        >
          {firstLetterUppercase(langues)}
        </p>
      {/each}
    </div>
  </div>
  <div class="accessibility">
    <p on:click={() => isTouchScreen.set(!$isTouchScreen)}>
      {`${
        $isTouchScreen
          ? $DataLanguages["footer"]["accessibilityDisabled"]
          : $DataLanguages["footer"]["accessibilityEnabled"]
      } ${$DataLanguages["footer"]["accessibility"]}`}
    </p>
  </div>
  <div class="licence">
